...
public class SanPhamRespository {
    ...
    public List<SanPhamViewModel> getAll() {
        String query = "select QuocGiaDong.id ,anh ,ten_hien_thi,COUNT(IMEI),moi,gia_ban,gia_nhap,mo_ta from QuocGiaDong join ChiTietDienThoai on QuocGiaDong.id = ChiTietDienThoai.id_quoc_gia_Dong\n"
                + "                where trang_thai ='true'\n"
                + "                group by anh ,ten_hien_thi,moi, QuocGiaDong.id,gia_ban,gia_nhap,mo_ta ";
        ...
        try ( Connection con = SQLServerConnection.getConnection();  PreparedStatement ps = con.prepareStatement(query);) {
            ...
            while (rs.next()) {
                ...
                SanPhamViewModel sp = new SanPhamViewModel(rs.getString(1), rs.getString(2),
                        rs.getString(3), rs.getInt(4), rs.getInt(5) == 100 ? "Mới" : "Cũ", rs.getDouble(6), rs.getDouble(7), rs.getString(8));
